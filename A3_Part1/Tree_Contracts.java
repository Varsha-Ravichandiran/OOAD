...
class AbsTree {
	...
	boolean member(int n) {
		return
			  (value == n) 
			  	||
			  (value < n && right.member(n))
				||
			  (value > n && left.member(n)
			  );
	}
	...
	@Requires("this.value != n || this.left != null || this.right != null") //added by varsha
...
class DupTree extends AbsTree {
	...
	@Ensures("n_count >= old_n_count && result == true")
	public boolean insert(int n) {
		old_n_count = get_count();
		if(old_n_count>1)
			n_count = old_n_count + 1;
		else if(old_n_count==1)
			n_count = old_n_count;
		return super.insert(n);
...
class AbsTree_Iterator {
	...
	@Requires("true")
	@Ensures({"(node != null && node.value == stack.peek().value) || node == null"})
